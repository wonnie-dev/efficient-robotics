...
3. Use the same manipulator platform as the confirmed real experiments in Professor Shinkyu Park's lab. The exact model is currently pending confirmation; do not silently assume a robot model merely because it is easier to simulate.
...
- Robot requirement: use the manipulator selected for confirmed real experiments in Professor Shinkyu Park's lab. Its exact model must be recorded before robot-specific simulation integration.
- Historical hardware context: SO-ARM 101 leader and follower arms were part of the broader project history, but SO-ARM 101 is not the required final robot.
...
- Public RT-2 artifacts were not directly aligned with SO-ARM 101.
...
A stronger but higher-risk extension is to predict how much each candidate action will reduce uncertainty before executing it. This could be implemented through:

- geometry-based visibility prediction;
- simulated camera rendering in Isaac Sim;
- a learned action-conditioned Scene Graph transition model;
- an approximate information-gain model.

This is promising but should not delay the core implementation.
...
The initial implementation may use sampling-based MPC or another robust controller available in Isaac Sim. The exact solver should be selected after inspecting the confirmed Professor Park lab robot model, control interface, and simulation stability.
...
- Confirm the exact Professor Park lab robot model and identify its URDF, USD, MJCF, meshes, and joint names.
...
- Ground plane.
- Table.
- A replaceable robot mount/interface until the Professor Park lab robot model is confirmed, then that exact manipulator.
- Fixed or wrist-mounted camera.
...
Count or rate of actions such as:

- picking the wrong object;
- opening the wrong container;
- grasping before the relation is sufficiently certain;
- moving an irrelevant occluder;
- repeatedly searching a disproven location;
- executing an action that violates the task relation.

The exact denominator must be fixed before experiments. Possible definitions are wrong actions per episode or wrong actions divided by all high-level actions.

## 14.3 Primary metric 3 - Information-Gathering Efficiency

Possible forms:

- Number of re-observation or interaction actions before success.
- Entropy reduction per action.
- Task-risk reduction per meter, second, or action.
- Total time or path length required to reach the decision threshold.

Use a simple definition in the main paper and place alternatives in supplementary material if needed.
...
- The final Isaac Sim setup must use the same manipulator platform as the confirmed real experiments in Professor Shinkyu Park's lab.
...
- Start with an open container and active viewpoint change.
- Add partial occlusion.
- Add a removable cover before a complex hinge lid.
- Use target and relation uncertainty in graph nodes and edges.
- Use task failure risk and expected uncertainty reduction in MPC action selection.
- Measure task success, wrong actions, and information-gathering efficiency as primary outcomes.
...
- Exact MPC solver.
- Whether to use a learned action-conditioned belief model or geometry-based prediction.
- Exact Professor Park lab robot model, gripper, and camera configuration.
- Final real-robot hardware available in Professor Park’s lab.
- Final paper title.
...
“Create an implementation plan for a minimal open-container active-viewpoint simulation using a replaceable robot interface until the Professor Park lab robot model is confirmed. Identify files to add or change, tests, expected outputs, and rollback checkpoints. Do not implement until the plan is reviewed.”

## Step 3 - build the minimal scene

Prompt example:

“Implement the smallest deterministic Isaac Sim scene with a replaceable robot mount, a table, an open basket, two objects, and a camera. Integrate the exact Professor Park lab manipulator after its model is confirmed. Add a script that saves one RGB image and scene metadata. Run it and report exact commands and outputs.”
...
- Prior chat records about RT-2, SO-ARM 101, Isaac Sim, server setup, uncertainty, metrics, and ICRA positioning.
