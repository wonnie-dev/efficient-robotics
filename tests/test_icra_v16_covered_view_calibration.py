...
from run_icra_v13_joint_live_development import add_covered_view_likelihoods


class IcraV16CoveredViewCalibrationTests(unittest.TestCase):
    def test_supplement_does_not_overlap_reserved_splits(self) -> None:
        config = json.loads(
            (
                ROOT
                / "configs/research/icra_v16_covered_view_supplemental_calibration_24episode.json"
    ...
    def test_covered_rows_are_added_without_replacing_open_rows(self) -> None:
        base = json.loads(
            (
                ROOT
                / "outputs/calibration/icra_v16_plus_supplement_joint_calibration_cv/calibration_candidate_model.json"
            ).read_text(encoding="utf-8")
        )
