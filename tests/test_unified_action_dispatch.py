"""CPU checks for the unified root-action physical dispatcher."""
...
class UnifiedActionDispatchTests(unittest.TestCase):
    def setUp(self) -> None:
        self.policy = {
    ...
    def test_closed_loop_config_declares_unified_action_scope(self) -> None:
        config = json.loads(
            (
                ROOT
                / "configs/research/closed_loop_episode.json"
    ...
    def test_server_accepts_each_terminal_action_at_the_root(self) -> None:
        source = (ROOT / "scripts/isaac_sim_server.py").read_text(
            encoding="utf-8"
        )
        for action in (
            '"grasp"',
