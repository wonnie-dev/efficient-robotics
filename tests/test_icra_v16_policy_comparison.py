...
from evaluate_icra_v16_policy_comparison import (  # noqa: E402
    DEFAULT_FREEZE_ROOT,
    DEFAULT_DEFINITIONS,
    DEFAULT_OUTPUT_ROOT,
...
class IcraV16PolicyComparisonTests(unittest.TestCase):
    ...
    def test_declared_method_set_is_complete(self) -> None:
        definitions = json.loads(
            (ROOT / "configs/research/icra_v16_method_definitions.json").read_text(
                encoding="utf-8"
            )
        )
        expected_methods = {
    ...
    def test_final_evaluator_uses_only_v16_paths_and_seed_split(self) -> None:
        for path in (DEFAULT_PERCEPTION_ROOT, DEFAULT_FREEZE_ROOT, DEFAULT_OUTPUT_ROOT):
            self.assertIn("icra_v16", str(path))
            self.assertNotIn("icra_v15b", str(path))
