#!/usr/bin/env python3
"""Cross-validate the V15 direct joint observation model and MPC replay.
...
DEFAULT_PERCEPTION_ROOT = (
    ROOT / "outputs/calibration/icra_v15_calibration_36episode_perception"
)
DEFAULT_OUTPUT_ROOT = ROOT / "outputs/calibration/icra_v15_joint_calibration_cv"
PREFLIGHT_MODEL = ROOT / "configs/research/icra_v15_unified_task_belief_preflight.json"
HYPOTHESES = (
...
def fit_model(
    ...
    method_version = (
        "icra-v16"
        if str(preflight.get("schema_version", "")).startswith("icra-v16")
        else "icra-v15"
    )
    model: dict[str, Any] = {
        "schema_version": f"{method_version}-direct-joint-observation-model-v1",
    ...
    *,
    ...
    blockers.extend(
        [
            ...
            "conditional_rg6_grasp_success_not_measured_on_v15_calibration_split",
    ...
    result = {
        "schema_version": (
            "icra-v16-joint-calibration-cv-result-v1"
            if str(preflight.get("schema_version", "")).startswith("icra-v16")
            else "icra-v15-joint-calibration-cv-result-v1"
        ),
