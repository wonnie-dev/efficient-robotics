...
CONFIG = ROOT / "configs/research/five_scenario_live_regression.json"
...
def run_job(
    ...
    effective = {
        **base,
        "schema_version": "episode-batch-job-v1",
        "status": "nonreserved_live_action_branch_diagnostic",
        "experiment_id": f"five_scenario_{job['headline_scenario']}_seed{seed}",
        "seed": seed,
    ...
    result_paths = sorted(job_root.glob(f"seed{seed:03d}/run*/closed_loop_result.json"))
...
def main(config_path: Path = CONFIG) -> None:
    ...
    state: dict[str, Any] = {
        "schema_version": "episode-batch-status-v1",
        "status": "running",
