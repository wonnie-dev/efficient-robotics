#!/usr/bin/env python3
"""Run calibration inference as independent single-GPU shards."""
...
CAPTURE_CONFIG = ROOT / "configs/research/unified_calibration_episodes.json"
CAPTURE_ROOT = ROOT / "outputs/live_pipeline/unified_calibration_episodes"
OUTPUT_ROOT = ROOT / "outputs/calibration/perception"
RUNNER = ROOT / "scripts/run_calibration_perception.py"
...
def main() -> None:
    ...
    if args.reserved_test:
        ...
        if freeze.get("status") != "frozen_ready_for_reserved_test":
            raise RuntimeError("The calibration package is not frozen")
    ...
    state: dict[str, Any] = {
        "schema_version": (
            "reserved-test-perception-batch-status-v1"
            if args.reserved_test
            else "calibration-perception-batch-status-v1"
        ),
        "status": "running",
    ...
    from run_calibration_perception import build_config
    ...
    print(f"CALIBRATION_PERCEPTION={output_root / 'COMPLETED.json'}")
