#!/usr/bin/env python3
"""Launch independent direct-VLM action inference shards and evaluate them."""
...
RUNNER = ROOT / "scripts/run_icra_v20_direct_vlm_action_inference.py"
EVALUATOR = ROOT / "scripts/evaluate_icra_v20_direct_vlm_action_baseline.py"
PYTHON = Path(
    os.environ.get(
...
def main() -> None:
    ...
    if not gpu_ids or len(gpu_ids) != len(set(gpu_ids)) or not set(gpu_ids) <= {0, 2, 4, 5}:
        raise ValueError("Only unique physical GPUs 0, 2, 4, and 5 are allowed")
    output_root = resolve_path(config["output_root"])
    logs = output_root / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    state = {
        "schema_version": "icra-v20-direct-vlm-action-sharded-status-v1",
        "status": "running",
    ...
    for shard_index, gpu in enumerate(gpu_ids):
        ...
        environment.update({"CUDA_VISIBLE_DEVICES": str(gpu), "CUDA_DEVICE_ORDER": "PCI_BUS_ID", "NVIDIA_VISIBLE_DEVICES": str(gpu), "PHYSICAL_GPU": str(gpu)})
        for name in ("RANK", "LOCAL_RANK", "WORLD_SIZE", "MASTER_ADDR", "MASTER_PORT"):
            environment.pop(name, None)
        command = [str(PYTHON), str(RUNNER), "--config", str(config_path), "--shard-index", str(shard_index), "--shard-count", str(len(gpu_ids))]
    ...
    for shard_index, gpu, process, stdout, stderr, stdout_path, stderr_path in jobs:
        try:
            ...
            stdout.close()
            stderr.close()
        record = {"shard_index": shard_index, "physical_gpu": gpu, "status": "completed" if returncode == 0 else "failed", "returncode": returncode, "stdout": str(stdout_path.resolve()), "stderr": str(stderr_path.resolve())}
        ...
        state.update(
            {
                ...
                "evaluation_skipped_because_subset": True,
            }
        )
        write_json(output_root / "status.json", state)
        print(json.dumps(state, indent=2))
        return
    evaluation = subprocess.run([str(PYTHON), str(EVALUATOR), "--config", str(config_path)], cwd=ROOT, text=True, capture_output=True, check=False)
