"""Generate deterministic VLM outputs for interface tests."""
...
def build_synthetic_output(model_input: dict) -> dict:
    sample_id = model_input["sample_id"]
    candidate_ids = [candidate["candidate_id"] for candidate in model_input["candidates"]]
    return {
        "schema_version": "vlm-output-v1",
        "sample_id": sample_id,
        "model": {"name": "deterministic_interface_fixture", "checkpoint": "none"},
        "target": {
        ...
        "provenance": {
            "prompt_version": "interface-fixture-v1",
...
def main() -> None:
    ...
    output = args.output or args.input.with_name("synthetic_output.json")
    output.write_text(
        json.dumps(build_synthetic_output(model_input), indent=2) + "\n",
        encoding="utf-8",
    )
