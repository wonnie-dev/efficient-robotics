#!/usr/bin/env python3
"""Capture one development scene for the v6 action-differentiating smoke."""
...
CONFIG = ROOT / "configs/research/icra_v6_stress_scene_smoke.json"
...
def assignment_for_seed(config: dict[str, Any], seed: int) -> dict[str, Any]:
    matches = [row for row in config["smoke_assignments"] if int(row["seed"]) == seed]
    if len(matches) != 1:
        raise ValueError(f"Expected one smoke assignment for seed {seed}, found {len(matches)}")
    return matches[0]
...
def completed_run(config: dict[str, Any], assignment: dict[str, Any]) -> tuple[Path, dict[str, Any]] | None:
    ...
    for result_path in sorted(seed_root.glob("run*/remove_cover_smoke_result.json"), reverse=True):
...
def main() -> None:
    ...
    state = {
        "schema_version": "icra-v6-stress-scene-smoke-status-v1",
        "status": "running",
    ...
    if found is None:
        ...
        write_json_atomic(status_path, state)
        raise RuntimeError(f"Stress smoke seed {args.seed} failed; see {log_path}")
    ...
    state.update(
        {
            ...
            "result_sha256": sha256(run_dir / "remove_cover_smoke_result.json"),
    ...
    write_json_atomic(status_path, state)
    print(f"V6_STRESS_SMOKE={args.seed}:{state['status']}:{run_dir}")
