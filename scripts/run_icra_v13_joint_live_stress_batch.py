#!/usr/bin/env python3
"""Run the V13 18-episode live development stress test on GPUs 0-3."""
...
CONFIG = ROOT / "configs/research/icra_v13_joint_live_stress_18seed.json"
...
def completed_result_exists(output_root: Path, seed: int) -> bool:
    ...
    candidates = sorted(
        (output_root / "episodes" / f"seed{seed}").glob(
            "run*/icra_v13_joint_live_result.json"
        )
    )
...
def main() -> None:
    ...
    state = {
        "schema_version": "icra-v13-joint-live-stress-batch-status-v1",
        "status": "running",
