...
from run_qwen_belief_mpc_replay import (
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Qwen belief adapter instead of the legacy threshold pilot."
        ),
    )
    parser.add_argument(
    ...
    try:
        ...
        if terminal_action == "grasp" and args.execute_contact_grasp:
            ...
            grasp_command = [
                str(ISAAC_PYTHON),
                str(ROOT / "scripts" / "run_rg6_actual_contact_grasp.py"),
                "--headless",
                "--renderer-gpu",
                "5",
                "--physics-gpu",
        ...
        result = {
            ...
            "controller_mode": (
                ...
                if belief_mpc_config is not None
                else "legacy_confidence_threshold_pilot"
            ),
            "valid_for_final_evaluation": False,
        }
