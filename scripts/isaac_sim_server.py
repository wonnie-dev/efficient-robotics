...
parser.add_argument(
    ...
    help=(
        ...
        "capture under a separate tabletop-scene directory."
    ),
)
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Tabletop scene only: replace the procedural slatted basket with the "
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Scanned-basket scene only: enlarge and raise the orange occluder so "
...
parser.add_argument(
    ...
    help=(
        "Scanned-basket scene only: deterministic factorized calibration scene. "
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Scanned-basket scene only: add a documented static bottom-and-four-"
...
parser.add_argument(
    ...
    default=0.0,
    help=(
        "Physics validation only: stop at a validated higher descent waypoint "
        "to grasp the upper part of a tall target inside a narrow container."
    ),
)
parser.add_argument(
    "--validate-rg6-physics",
    action="store_true",
    help=(
...
if args.overview_demo and (
    ...
    parser.error("Overview demo cannot be combined with another execution mode")
if args.overview_demo and args.scene_profile != "benchmark":
    parser.error("--overview-demo requires --scene-profile benchmark")
...
if args.seeded_scene_capture and (
    args.execute_non_oracle_plan
    or args.execute_action_request
    or args.live_pipeline_server
):
    parser.error("Seeded scene capture cannot be combined with another execution mode")
if args.seeded_scene_capture and args.scene_profile != "benchmark":
    parser.error("--seeded-scene-capture requires --scene-profile benchmark")
if args.tabletop_scene and not (
    args.seeded_scene_capture or args.live_pipeline_server
):
    parser.error(
        "--tabletop-scene requires --seeded-scene-capture "
        "or --live-pipeline-server"
    )
if (
    args.scanned_basket_scene
    and not args.tabletop_scene
):
    parser.error(
        "--scanned-basket-scene requires "
        "--tabletop-scene"
    )
if args.active_occlusion_scene and not args.scanned_basket_scene:
    parser.error(
...
if args.active_occlusion_scene and args.calibration_scene_variant is not None:
    parser.error(
...
coordinated_target_physics_validation = (
    args.execute_persistent_composite_grasp
...
if args.rg6_coupling_mode == "coordinated_drives" and not (
    ...
    and (coordinated_cover_proxy or coordinated_target_physics_validation)
):
    parser.error(
        "--rg6-coupling-mode coordinated_drives requires either provisional "
        "remove-cover execution or a target-only persistent physics validation, "
...
if args.rg6_physics_validation and args.scene_profile != "benchmark":
    parser.error("--validate-rg6-physics requires --scene-profile benchmark")
if args.rg6_physics_validation and (
    args.execute_non_oracle_plan
    or args.execute_action_request
    ...
    parser.error("--validate-rg6-physics cannot be combined with another mode")
if args.seed < 0:
...
if args.overview_demo or args.candidate_view_demo or args.live_pipeline_server:
    quality_settings = {
        **quality_settings,
        "anti_aliasing": 4,
        "capture_steps": 3,
...
if (
    ...
    if args.tabletop_scene:
        from tabletop_scene_assets import apply_tabletop_scene_assets

        household_scene = apply_tabletop_scene_assets(
            stage, seeded_layout
        )
        ...
        if args.scanned_basket_scene:
            ...
            household_scene["reference"] = (
                replace_procedural_basket_with_scan(
                    stage,
                    active_occlusion_scene=args.active_occlusion_scene,
...
if args.actual_view_motion:
    ...
    composite_asset_value = Path(import_result["output_usd"])
    composite_asset = (
        composite_asset_value.resolve()
        if composite_asset_value.is_absolute()
        else (PROJECT_ROOT / composite_asset_value).resolve()
    )
...
if args.tabletop_scene and args.seeded_scene_capture:
    # This first household-shaped scene intentionally evaluates exactly the
...
elif args.overview_demo or args.candidate_view_demo or args.live_pipeline_server:
    high_quality_overview = observation_config["overview_camera"][
        "high_quality"
    ]
    observation_config["overview_camera"]["resolution"] = high_quality_overview[
        "resolution"
    ]
    observation_config["overview_camera"]["focal_length_mm"] = (
        high_quality_overview["focal_length_mm"]
    )
...
if args.rg6_physics_validation:
    rg6_base_path = (
...
elif args.seeded_scene_capture:
    output_family = (
        "tabletop_scene"
        if args.tabletop_scene
        else "seeded_scene"
...
if args.rg6_physics_validation:
    ...
    diagnostics = {
        "schema_version": "rg6-physical-articulation-validation-v1",
        "status": "completed" if finger_dof_names else "failed",
        "articulation_mode": (
            "rg6_physics_articulation_with_world_fixed_base"
        ),
    ...
    diagnostics_path = diagnostics_root / "articulation_validation.json"
    diagnostics_path.write_text(
        json.dumps(diagnostics, indent=2) + "\n",
        encoding="utf-8",
    )
    record(f"RG6_ARTICULATION_VALIDATION={diagnostics_path}")
    print(f"RG6_ARTICULATION_VALIDATION={diagnostics_path}", flush=True)
    app_utils.stop()
    simulation_app.close()
...
if args.live_pipeline_server:
...
elif args.overview_demo:
    from PIL import Image, ImageDraw, ImageFont

    from build_observation_video import build_frame_sequence_video

    planner_result = json.loads(
        args.planner_result.resolve().read_text(encoding="utf-8")
    )
    if planner_result.get("status") != "completed":
        raise RuntimeError("Planner result is not completed")
    first_action = planner_result["first_action"]["type"]
    if not first_action.startswith("viewpoint_"):
        raise RuntimeError(f"Overview demo requires a viewpoint action: {first_action}")
    selected_pose = first_action.removeprefix("viewpoint_")
    if selected_pose not in observation_config["capture"]["poses"]:
        raise RuntimeError(f"Unknown overview-demo viewpoint: {selected_pose}")
    replanned_action = planner_result["replan"]["type"]

    demo_root = PROJECT_ROOT / "outputs" / "overview_demo" / "benchmark_seed000"
    frame_root = demo_root / "overview_frames"
    frame_root.mkdir(parents=True, exist_ok=True)
    frame_paths = []
    frame_counter = {"value": 0}
    ...
    def save_demo_frame(label: str) -> None:
        rep.orchestrator.step(rt_subframes=2)
        rgba = np.asarray(overview_rgb_annotator.get_data()).copy()
        image = Image.fromarray(rgba[:, :, :3].astype(np.uint8), "RGB")
        draw = ImageDraw.Draw(image)
        text_box = draw.textbbox((0, 0), label, font=overlay_font)
        text_width = text_box[2] - text_box[0]
        text_height = text_box[3] - text_box[1]
        draw.rounded_rectangle(
            (18, 18, 42 + text_width, 38 + text_height),
        ...
        draw.text((30, 26), label, fill=(255, 255, 255), font=overlay_font)
        frame_path = frame_root / f"frame_{frame_counter['value']:04d}.png"
        image.save(frame_path)
        frame_paths.append(frame_path)
        frame_counter["value"] += 1
    ...
    simulation_app.update()
    if trajectory_result["status"] != "completed":
        raise RuntimeError(f"Overview trajectory failed: {trajectory_result}")
    capture_observation(selected_pose)
    for _ in range(12):
        save_demo_frame(
    ...
    if replanned_action.startswith("viewpoint_"):
        ...
        if replanned_pose not in observation_config["capture"]["poses"]:
            raise RuntimeError(
                f"Unknown replanned overview-demo viewpoint: {replanned_pose}"
            )
        ...
        if replanned_trajectory["status"] != "completed":
            raise RuntimeError(
                "Overview replanned trajectory failed: "
    ...
    video_result = build_frame_sequence_video(
        frame_paths,
        demo_root / "active_view_demo.mp4",
        fps=5,
        crf=quality_settings["video_crf"],
        preset=quality_settings["video_preset"],
        purpose=(
            "cached_qwen_active_view_pipeline_overview_demo_"
            "not_final_evaluation"
        ),
    )
    overview_demo_result = {
        "schema_version": "active-view-demo-v1",
        "status": "completed",
        "planner_result": str(args.planner_result.resolve()),
        "initial_observation": "center",
        ...
        "limitations": (
            ...
            "deterministic validation run. Both robot viewpoint motions and all RGB-D "
            "captures are executed live in Isaac Sim. No third Qwen inference "
            "or post-left replan is performed."
        ),
    }
    demo_result_path = demo_root / "overview_demo.json"
    demo_result_path.write_text(
        json.dumps(overview_demo_result, indent=2) + "\n",
        encoding="utf-8",
    )
    record(
        "OVERVIEW_DEMO_COMPLETED="
    ...
    execution = {
        ...
        "capture_files": [
            ...
            f"outputs/observations/{selected_pose}/uncertainty_scene_graph.json",
        ],
        "limitations": (
...
if overview_demo_result is not None:
    print(
        "OVERVIEW_DEMO_VIDEO="
        + overview_demo_result["video"]["output_path"]
    )
    print(
        "OVERVIEW_DEMO_REPLAN="
        + overview_demo_result["replanned_action"]
    )
