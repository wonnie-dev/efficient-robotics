"""Run a cache-first, inference-only VLM episode.

This runner is intentionally limited to sequential, batch-size-one inference.
It converts uncalibrated Qwen raw logits into uncalibrated beliefs, selects one
...
DEFAULT_CACHE_ROOT = ROOT / "outputs" / "inference_cache" / "qwen3_vl"
DEFAULT_OUTPUT_ROOT = ROOT / "outputs" / "single_gpu_inference"
...
def fuse_beliefs(prior: dict, observation: dict) -> dict:
    ...
    return {
        ...
        "calibrated": False,
        "source": (
            "product_fusion_of_uncalibrated_view_probabilities"
        ),
    }
...
def select_action(
    ...
    if (
        ...
        action = "grasp"
        reason = "confidence_thresholds_satisfied"
    else:
        next_view = next(
            (
                view
        ...
        reason = (
            ...
            if next_view
            else "no_unvisited_view_available"
        )
    return {
        "type": action,
        "reason": reason,
...
def main() -> None:
    ...
    episode_result = {
        "schema_version": "single-gpu-inference-episode-v1",
        "status": "completed",
