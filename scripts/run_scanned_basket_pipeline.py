"""Run one causal learned-perception-to-contact-grasp episode.
...
from evaluate_multiview_belief import planner_config
...
MODEL_CONFIG_SOURCE = (
    ...
    / "configs"
    / "perception"
    / "grounded_segmentation.json"
)
PLANNER_CONFIG_SOURCE = (
    ROOT
    / "configs"
    / "research"
    / "multiview_belief.json"
)
OUTPUT_ROOT = (
    ROOT
    / "outputs"
...
GROUNDED_QWEN_CACHE_ROOT = (
    ROOT / "outputs" / "inference_cache" / "grounded_qwen_factorized"
)
...
def make_perception_config(
    *,
    ...
    config["limitations"] = [
        "Single deterministic live integration episode.",
...
def main() -> None:
    ...
    allow_validation_grasp = os.environ.get("ALLOW_VALIDATION_GRASP") == "1"
    if args.execute_grasp and args.seed != 0 and not allow_validation_grasp:
        raise ValueError(
