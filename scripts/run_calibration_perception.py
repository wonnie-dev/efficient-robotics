#!/usr/bin/env python3
"""Run pretrained GroundingDINO, SAM2, and Qwen on calibration views."""
...
from capture_calibration_batch import (  # noqa: E402
    assignments,
    completed,
)


CAPTURE_CONFIG = ROOT / "configs/research/calibration_episodes.json"
CAPTURE_ROOT = ROOT / "outputs/live_pipeline/calibration_episodes"
OUTPUT_ROOT = ROOT / "outputs/calibration/calibration_episodes_perception"
VALIDATION_ROOT = ROOT / "outputs/calibration/perception_validation"
MODEL_PATH = Path(
    os.environ.get(
    ...
    for row in assignments(capture):
        ...
        if result_path is None:
            raise FileNotFoundError(
                f"No accepted calibration capture for seed {row['seed']}"
            )
...
def main() -> None:
    ...
    output_root = (
        ...
        else (VALIDATION_ROOT if args.sample_limit is not None else OUTPUT_ROOT)
    )
    ...
    if args.prepare_only:
        print(f"CALIBRATION_PERCEPTION_PREPARED={config_path}")
        return
    ...
    state: dict[str, Any] = {
        "schema_version": "perception-status-v1",
        "status": "running",
    ...
    commands = (
        ("gdino_detect", [python, str(ROOT / "scripts/run_grounded_segmentation.py"), "gdino_detect", "--config", str(config_path)], max(900.0, sample_count * 15.0)),
        ("sam2_segment", [python, str(ROOT / "scripts/run_grounded_segmentation.py"), "sam2_segment", "--config", str(config_path)], max(900.0, sample_count * 15.0)),
    ...
    try:
        ...
        print(f"CALIBRATION_PERCEPTION_COMPLETED={output_root / 'COMPLETED.json'}")
