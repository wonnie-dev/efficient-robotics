...
def main() -> None:
    ...
    if args.active_occlusion_pilot:
        command.append("--active-occlusion-pilot")
        ...
        try:
            ...
            for index, action_type in enumerate(requests):
                ...
                write_json_atomic(
                    ...
                    {
                        "schema_version": "actual-view-motion-smoke-request-v1",
                        "index": index,
                        "type": action_type,
    ...
    write_json_atomic(output_dir / "smoke_result.json", result)
    if not success:
        raise RuntimeError(f"Actual view-motion smoke failed: {result}")
    print(f"ACTUAL_VIEW_MOTION_SMOKE={output_dir / 'smoke_result.json'}")
