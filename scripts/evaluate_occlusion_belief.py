"""Evaluate action-conditioned belief updates under controlled occlusion.
...
def main() -> None:
    ...
    if not selected_action.startswith("viewpoint_"):
        raise RuntimeError(
            f"Evaluation expected a re-observation action, got {selected_action}"
        )
    ...
    result = {
        "schema_version": "occlusion-belief-evaluation-v1",
        "status": "completed",
