"""Run GroundingDINO and SAM2 on configured RGB observations.
...
def load_config(path: Path) -> dict[str, Any]:
    """Load the inference contract and reject any training-enabled variant."""
    config = json.loads(path.read_text(encoding="utf-8"))
    if config.get("training_performed") is not False:
        raise ValueError("This runner must remain inference-only.")
    return config
...
def sample_items(config: dict[str, Any], limit: int | None) -> list[dict[str, Any]]:
    """Return configured samples, optionally truncated for a validation run."""
